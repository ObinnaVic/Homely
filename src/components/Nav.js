...
import { CLOSECART, CLOSENAV, CLOSE_ORDER_MODAL, CLOSE_SUBMIT, OPENCART, OPENNAV, OPEN_ORDER_MODAL, SUBMITORDER } from "./Action";
    ...
    <main>
      ...
      <div className={cart ? "cart secondary-color p-3" : "d-none"}>
        ...
        <div className="d-flex justify-content-between align-items-center">
          ...
          <button className="btn primary-color" onClick={() => openOrder()}>
            Place Order
          </button>
        </div>
      </div>
        ...
        <div className="modal-info d-flex flex-column align-items-center">
          ...
          <form>
            <input
              ...
              style={{ width: "100%", padding: "3px", margin: "3px" }}
            />
            <input
              type="number"
              name="phone number"
              ...
              style={{ width: "100%", padding: "3px", margin: "3px" }}
            />
            <h6>Desired time of delivery</h6>
            <input type="time" name="time" />
          </form>
          <button className="primary-color m-2 p-1" onClick={() => submit()}>
            Submit
          </button>
        </div>
      </div>
...
const mapStateToProps = (store) => {
    const {amount, nav, data, total, cart, orderModal, submitOrder} = store;
    return {amount, nav, data, total, cart, orderModal, submitOrder};
}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    submit: () => dispatch({type: SUBMITORDER}),
    closeSubmit: () => dispatch({type: CLOSE_SUBMIT}),
  };
};
